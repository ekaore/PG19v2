  ...
  switch (e.key) {
    ...
    case 'Enter':
      ...
      if (selectedIndex.value >= 0 && suggestions.value[selectedIndex.value]) {
        ...
        // Преобразуем readonly координаты в mutable
        const mutableSuggestion: DadataSuggestion = {
          ...suggestion,
          coordinates: suggestion.coordinates ? [suggestion.coordinates[0], suggestion.coordinates[1]] as [number, number] : null
        }
        selectSuggestion(mutableSuggestion)
      }
      break
    case 'Escape':
      showSuggestions.value = false
