  ...
  >
    ...
    <div class="container mx-auto px-4">
      <div class="flex items-center justify-between h-16">
        ...
        <nav class="hidden lg:flex items-center gap-1" aria-label="Услуги">
          <NuxtLink
          ...
          >
            <Icon
              :name="item.icon"
              class="w-5 h-5 transition-colors duration-300 group-hover:text-primary group-hover:scale-110"
            />
            <span class="font-medium">{{ item.name }}</span>
            <!-- Индикатор активной страницы -->
            <span
              class="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-primary transition-all duration-300 group-hover:w-3/4"
              :class="{ 'w-3/4': $route.path === item.href }"
            />
          </NuxtLink>
        </nav>
